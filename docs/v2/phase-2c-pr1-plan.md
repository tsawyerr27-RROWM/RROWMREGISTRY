...
Plan **Phase 2C PR1 — Opportunity Foundation**: the first implementation train after governance approval. PR1 establishes **programme and brief publishing** and **public open-calls discovery** — the read path for opportunities — without applications, awards, or commissions.

**PR1 north-star:**

> Organisations can **draft and publish** programmes and briefs in Studio; anonymous visitors can **discover and read** published briefs on Field through explainable filters — with no apply flow, no inbox, and no commission objects yet.
...
| **Brief (Studio)** | Org admin/staff create, edit, draft, publish, withdraw briefs; participation mode selection; brief types (open call, residency/award, direct, fabrication RFP) |
| **Publishing gates** | Verification + subscription policy enforced at publish (per founder freeze) |
| **Programme Field page** | `/field/programmes/[slug]` — published programme + linked briefs |
| **Brief Field page** | `/field/open-calls/[id]` — scope, practices, registry outcome, org context |
| **Open calls index** | `/field/open-calls` — filterable listing (explainable filters only) |
| **Explorer IA hook** | Open calls entry on Field hub — Records default unchanged |
| **Graph edges** | Brief → org; brief → programme; programme → briefs |
...
**Placeholder rule:** PR1 brief pages show **what** the opportunity is — not a broken apply button. Copy may state that applications open in a subsequent release only if founder approves interim messaging; default is **read-only opportunity surfaces**.
...
|-------|--------------|
| **AC-PG*** | AC-PG1–AC-PG5 |
| **AC-BR*** | AC-BR1–AC-BR5, AC-BR7 (direct/apply deferred partial — AC-BR6, AC-BR8 → PR2) |
| **AC-OC*** | AC-OC1–AC-OC6 |
| **AC-GN*** | AC-GN1–AC-GN3 (opportunity edges only); AC-GN4 regression |
| **AC-VT*** | AC-VT1–AC-VT2 (brief surfaces) |
...
Step 0  Governance lock (founder freeze + spec LOCKED)
    │
    └── Step 1  PR1 — Programmes + briefs + open calls read path  ← this plan
              │
...
| 1.2 | Studio brief CRUD + participation mode + types | AC-BR1–AC-BR3 |
| 1.3 | Publish gates (verification, subscription policy) | AC-BR7 |
| 1.4 | Field programme page | AC-PG3–AC-PG5 |
| 1.5 | Field brief detail page | AC-BR4 |
| 1.6 | Open calls index + filters | AC-OC1–AC-OC6 |
| 1.7 | Hub IA + graph edges | AC-GN1–AC-GN3 |
| 1.8 | Trust copy on brief surfaces | AC-VT1–AC-VT2 |
...
|-------------|--------------|
| Practice taxonomy | Brief practice requirements display + filter |
| Organisation presence | Publisher context on brief pages |
| Field explorer IA | Hub extension for open calls |
...
| Open calls filters | Explainable AND composition only |
...
| Staging QA: publish brief → visible on open calls | Product |
