-- Personal Archive: participant-maintained cultural memory (not social engagement).
...
-- Reserved for future archive update surfacing (notifications not implemented in v1).
...
-- Aggregate counts only — never expose archivist identities.
...
-- Refresh PostgREST schema cache (avoids "Could not find table … in the schema cache").
