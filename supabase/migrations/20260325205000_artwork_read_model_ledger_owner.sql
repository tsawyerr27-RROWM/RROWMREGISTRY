-- Expose latest ledger owner on the read model.
-- IMPORTANT: ownership_events column names vary across deployments. We build the
-- view dynamically based on which "to owner" column exists.
...
begin
  if exists (
    ...
    -- As a last resort, skip ledger owner (view still works).
    owner_to_col := null;
  end if;
