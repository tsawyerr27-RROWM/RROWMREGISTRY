...
export function OpportunityExplorerPagination({ basePath, page, total, params }: Props) {
  const { t } = useLocalePreferences();
  const totalPages = Math.max(1, Math.ceil(total / FIELD_OPPORTUNITY_PAGE_SIZE));
  const start = total === 0 ? 0 : (page - 1) * FIELD_OPPORTUNITY_PAGE_SIZE + 1;
  const end = Math.min(page * FIELD_OPPORTUNITY_PAGE_SIZE, total);

  function hrefForPage(p: number) {
    const qs = fieldOpportunityQueryString({ ...params, page: p });
