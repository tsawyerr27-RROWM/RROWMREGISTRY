...
export async function POST(req: Request) {
  try {
    ...
    const galleryId = body.galleryId != null ? String(body.galleryId) : "";
    const verified = body.verified != null ? Boolean(body.verified) : true;
    if (!galleryId) {
    ...
    const { error } = await admin.rpc("admin_set_gallery_verified", {
    ...
    if (error) {
      console.error("[verify-gallery]", error.message);
