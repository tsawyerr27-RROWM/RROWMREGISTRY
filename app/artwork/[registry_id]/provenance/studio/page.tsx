...
import { studioContinuityNotesFromBundle } from "@/lib/registry-continuity";
import { maskArtistInviteEmail } from "@/lib/mask-email";
import { InfoTooltip } from "@/components/ui/InfoTooltip";

export const dynamic = "force-dynamic";

export default async function ProvenanceStudioPage({
  params,
}: {
    ...
    return (
      <div className="min-h-screen rrowm-bg-page pt-20 text-neutral-900">
        <main className="mx-auto max-w-3xl px-6 py-12 md:py-20">
          <h1 className="mt-10 font-serif text-[1.75rem] font-normal text-neutral-950">
            Studio actions
          </h1>
  ...
  const archivalBundle =
    artworkRow?.registry_id != null && artworkRow?.created_at
      ...
      : null;
  const studioContinuityLines = studioContinuityNotesFromBundle(archivalBundle);
    ...
    <div className="min-h-screen rrowm-bg-page pt-20 text-neutral-900">
      <main className="mx-auto max-w-3xl px-6 py-12 md:py-20">
        <header className="mt-10 border-b border-neutral-200/80 pb-10">
          ...
          <h1 className="font-serif text-[1.75rem] font-normal tracking-[-0.01em] text-neutral-950 md:text-3xl">
            Studio & verification actions
          </h1>
        ...
        {studioContinuityLines.length > 0 || pendingContinuationInvitee ? (
          <section
            className="mt-12 rounded-xl border border-neutral-200/70 bg-white/40 px-5 py-6 md:px-6"
            aria-label="Record continuity"
          >
            ...
            <div className="mt-4 max-w-xl space-y-3 text-[13px] leading-relaxed text-neutral-600">
              {studioContinuityLines.map((line) => (
                <p key={line}>{line}</p>
              ))}
